...
from auto_handlers import close_ticket, start_combined_test
...
async def process_control_command(message, self_bot):
    ...
    try:
        # ── فتح / إغلاق التخصصات ──
        if cmd == "!فتح_دمج":
            config.EDIT_WHITENING_OPEN = True
            await message.channel.send("✅ **تحرير + تبييض:** تم فتح التقديم.")
        elif cmd == "!اغلاق_دمج":
            config.EDIT_WHITENING_OPEN = False
            await message.channel.send("🔒 **تحرير + تبييض:** تم إغلاق التقديم.")
        elif cmd == "!فتح_ترجمة":
            config.TRANSLATE_OPEN = True
            await message.channel.send("✅ **ترجمة:** تم فتح التقديم.")
        elif cmd == "!اغلاق_ترجمة":
            config.TRANSLATE_OPEN = False
            await message.channel.send("🔒 **ترجمة:** تم إغلاق التقديم.")
        elif cmd == "!تبديل_دمج":
            config.EDIT_WHITENING_OPEN = not config.EDIT_WHITENING_OPEN
            state = "مفتوح" if config.EDIT_WHITENING_OPEN else "مغلق"
            await message.channel.send(f"🔄 **تحرير + تبييض:** أصبح {state}.")
        elif cmd == "!تبديل_ترجمة":
            config.TRANSLATE_OPEN = not config.TRANSLATE_OPEN
            state = "مفتوح" if config.TRANSLATE_OPEN else "مغلق"
            await message.channel.send(f"🔄 **ترجمة:** أصبحت {state}.")

        # ── عرض الحالة ──
        elif cmd == "!الحالة":
            edit_icon = "🟢" if config.EDIT_WHITENING_OPEN else "🔴"
            trans_icon = "🟢" if config.TRANSLATE_OPEN else "🔴"
            active = len(active_tests)
            txt = (
                f"**📊 حالة النظام**\n"
                f"{edit_icon} **تحرير + تبييض:** {'مفتوح' if config.EDIT_WHITENING_OPEN else 'مغلق'}\n"
                f"{trans_icon} **ترجمة:** {'مفتوح' if config.TRANSLATE_OPEN else 'مغلق'}\n"
                f"📁 **تكتات نشطة:** {active}"
            )
            await message.channel.send(txt)

        # ── عرض التكتات النشطة (بدون مرحلة) ──
        elif cmd == "!التكتات":
            if not active_tests:
                ...
                for idx, (ch_id, ttype) in enumerate(active_tests.items(), 1):
                    ...
                    ttype_display = "تحرير+تبييض" if ttype == "combined" else "ترجمة"
        ...
        # ── إرسال فشل لتكت محدد ──
            ...
            if args:
                ...
                await human_send_smart(ch, config.FAIL_MSG.replace("{mention}", mention))
        ...
        # ── ضبط المدد ──
            ...
            if args:
                config.COMBINED_TEST_DURATION_SEC = int(args[0]) * 3600
                await message.channel.send(f"⏱️ مدة تحرير + تبييض أصبحت **{args[0]}** ساعة.")
            else:
                await message.channel.send("❗ استخدم `!وقت_دمج <عدد الساعات>`")
        elif cmd == "!وقت_الترجمة":
            if args:
                config.TRANSLATE_TEST_DURATION_SEC = int(args[0]) * 3600
            ...
            if args:
                config.CLOSED_TICKET_CLOSE_DELAY = int(args[0]) * 60
            ...
            if args:
                config.INFO_CHANNEL_LINK = args[0]
        ...
        # ── عرض الإعدادات ──
            ...
            txt = (
                ...
                f"**تحرير + تبييض:** {'مفتوح' if config.EDIT_WHITENING_OPEN else 'مغلق'}\n"
                f"**ترجمة:** {'مفتوح' if config.TRANSLATE_OPEN else 'مغلق'}\n"
                f"**مدة التحرير + التبييض:** {config.COMBINED_TEST_DURATION_SEC//3600} ساعة\n"
                f"**مدة الترجمة:** {config.TRANSLATE_TEST_DURATION_SEC//3600} ساعة\n"
                f"**تأخير إغلاق التكت المغلق:** {config.CLOSED_TICKET_CLOSE_DELAY//60} دقيقة\n"
                f"**رابط المعلومات:** {config.INFO_CHANNEL_LINK}"
            )
            await message.channel.send(txt)
        ...
        # ── أوامر عامة ──
            ...
            category = self_bot.get_channel(config.CATEGORY_ID)
            if category:
        ...
        # ── مساعدة بالعربية ──
            ...
            help_txt = (
                ...
                "**⚙️ الإعدادات**\n"
                "`!وقت_دمج <ساعات>` – ضبط مدة تحرير + تبييض\n"
